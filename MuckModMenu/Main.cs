...
namespace MuckModMenu
{
    public class Main
    {
        ...
        static void LoadRPC(UnityModManager.ModEntry modEntry)
        {
            client = new DiscordRpcClient("875717521360187393")
            {
            ...
            {
                modEntry.Logger.Error("Failed loading RPC: " + e.Message);
            };

            client.OnReady += (sender, e) =>
            {
                modEntry.Logger.Log("Received Ready from user " + e.User.Username);
            };
            ...
            client.SetPresence(new RichPresence()
            {
                ...
                Details = "Unity Mod Library!",
                State = "Using the a ModMenu from Unity Mod Library!",
                Assets = new DiscordRPC.Assets()
                {
                    LargeImageKey = "large",
                    LargeImageText = "https://unitymodlibrary.tk",
                    SmallImageKey = "muck",
                    SmallImageText = "playing Muck with the Menu Version " + modEntry.Version + ""
                }

            });

            modEntry.Logger.Log("DiscordRPC has been finished!");
        }

        static void start(UnityModManager.ModEntry modEntry)
        {
            LoadRPC(modEntry);
        }
        ...
        static void stop()
        {
            if (client != null && client.IsInitialized && !client.IsDisposed) client.Dispose();
        }

        static bool OnToggle(UnityModManager.ModEntry modEntry, bool toggle)
        {
            ...
            static void OnUpdate(UnityModManager.ModEntry modEntry, float df)
        {
            ...
            if (client != null && client.IsInitialized && !client.IsDisposed) client.Invoke();
